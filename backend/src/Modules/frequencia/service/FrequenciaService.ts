...
import { AuthContextGateway, type ContextoFrequenciaGateway } from "../gateways/AuthContextGateway";
...
export class FrequenciaService {
  ...
  async listarOpcoes(req: Request) {
    ...
    return {
      ...
      turmas: turmas.map((t) => ({ id: t.id, turmaDisciplinaId: t.id, turmaId: t.turma_id, sigla: t.turma_sigla,
  ...
  async obterChamada(turmaId: string, data: string, req: Request) {
    ...
    const porMatricula = new Map(registros.map((r) => [String(r.matricula_turma_disciplina_id), r]));
    return { turmaDisciplinaId: turmaId, data, periodoLetivo: turma.periodo_codigo, jaRegistrada: registros.length > 0,
      ...
      alunos: await Promise.all(alunos.map(async (a) => { const r = porMatricula.get(String(a.matricula_turma_disciplina_id)); return {
  ...
  async salvarChamada(payload: RegistrarFrequenciaRequest, req: Request) {
    ...
    const elegiveisPorAluno = new Map(elegiveis.map((a) => [String(a.aluno_id), a]));
    ...
    try {
      const salvo = await this.repository.salvarChamadaAtomica({ turmaDisciplinaId: payload.turmaDisciplinaId, aulaId: payload.aulaId,
        ...
        registros: payload.registros.map((r) => ({ matriculaId: elegiveisPorAluno.get(r.alunoId)?.matricula_turma_disciplina_id as string, status: r.status })) });
      return { mensagem: "Chamada salva com sucesso.", ...salvo };
    } catch (e: unknown) {
      if (e instanceof FrequenciaError) throw e;
      const erro = e as ErroChamada;
      if (erro?.code === "23505") throw erroFrequencia.conflito("Já existe uma chamada para esta turma e data.");
      if (erro?.codigoDominio === "PRAZO_EXPIRADO") throw erroFrequencia.conflito(erro.message);
      if (erro?.codigoDominio) throw erroFrequencia.invalido(erro.message);
      throw e;
    }
  }
  ...
  async registrarJustificativa(id: string, dados: JustificativaRequest, req: Request) {
    ...
    const registro = await this.repository.buscarRegistroPorId(id); if (!registro) throw erroFrequencia.naoEncontrado("Registro de frequência não encontrado.");
  ...
  async consultarAlunoInterno(alunoId: string, professorId?: string) {
    const historico = await this.repository.listarHistoricoAluno(alunoId, professorId);
  ...
  async consultarTurma(id: string, req: Request, filtros: FiltrosTurma = {}) {
    ...
    const turma: TurmaRow | undefined = await this.repository.buscarTurma(id);
    ...
    const alunos = rows.map((r) => this.consolidar({ alunoId: r.aluno_id, alunoNome: r.aluno_nome, turmaDisciplinaId: r.turma_disciplina_id, disciplinaId: r.disciplina_id, disciplinaNome: r.disciplina_nome, totalAulas, presencas: Number(r.presencas), faltas: Number(r.faltas), naoLancadas: Math.max(totalAulas - Number(r.registros), 0) }));
    return { turmaDisciplinaId: id, totalAulas, matriculasIrregulares: await this.repository.contarMatriculasIrregulares(id), alunos,
      alunosEmRisco: alunos.filter((a) => a.situacao === "RISCO_REPROVACAO"), alunosEmAlerta: alunos.filter((a) => a.situacao === "ALERTA") };
  }
  async gerarRelatorio(f: FiltrosTurma, req: Request) { const id = String(f.turmaDisciplinaId || ""); if (!id) throw erroFrequencia.invalido("Informe turma e disciplina."); return { filtros: { turmaDisciplinaId: id, dataInicio: f.dataInicio || null, dataFim: f.dataFim || null }, ...(await this.consultarTurma(id, req, f)) }; }

  private async autorizarTurma(ctx: ContextoFrequenciaGateway, id: string, secretariaPode = false) { if (ctx.perfil === "secretaria" && secretariaPode) return; if (ctx.perfil !== "professor" || !ctx.professorId || !(await this.repository.professorPossuiTurma(ctx.professorId, id))) throw erroFrequencia.proibido("Turma fora da atribuição do professor."); }
  private async validarPeriodo(id: string, data: string, escrita: boolean) { const t: TurmaRow | undefined = await this.repository.buscarTurma(id); if (!t) throw erroFrequencia.naoEncontrado("Turma/disciplina não encontrada."); if (data > this.hojeLocal()) throw erroFrequencia.invalido("Não é permitido registrar data futura."); if (data < this.iso(t.data_inicio) || data > this.iso(t.data_fim)) throw erroFrequencia.invalido("Data fora do período letivo da turma."); if (escrita && (!t.periodo_ativo || !["ativo", "ATIVO", "em_andamento", "EM_ANDAMENTO"].includes(t.periodo_status as string))) throw erroFrequencia.conflito("O período letivo não está ativo para lançamentos."); return t; }
  private validarPayload(p: RegistrarFrequenciaRequest) { if (!p || !Array.isArray(p.registros) || !p.registros.length) throw erroFrequencia.invalido("Informe a chamada completa."); this.uuid(p.turmaDisciplinaId, "Turma/disciplina inválida."); this.data(p.data); if (p.aulaId) this.uuid(p.aulaId, "Aula inválida."); if (p.localId) this.uuid(p.localId, "Local inválido."); p.registros.forEach((r) => { this.uuid(r.alunoId, "Aluno inválido."); this.status(r.status); }); }
  private filtros(f: FiltrosTurma) { if (f.dataInicio) this.data(String(f.dataInicio)); if (f.dataFim) this.data(String(f.dataFim)); if (f.dataInicio && f.dataFim && f.dataInicio > f.dataFim) throw erroFrequencia.invalido("A data inicial deve ser anterior à final."); }
  ...
  private consolidar(g: GrupoConsolidado): ConsolidadoFrequencia { const contabilizadas = g.presencas + g.faltas; const percentual = contabilizadas ? Number((g.presencas / contabilizadas * 100).toFixed(2)) : null; return { ...g, percentual, situacao: percentual === null ? "NAO_LANCADO" : percentual < 75 ? "RISCO_REPROVACAO" : percentual <= 80 ? "ALERTA" : "REGULAR" }; }
  private iso(v: unknown) { return v instanceof Date ? v.toISOString().slice(0, 10) : String(v).slice(0, 10); }
