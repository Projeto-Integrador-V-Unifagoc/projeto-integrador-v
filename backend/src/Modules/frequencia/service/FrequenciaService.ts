...
export class FrequenciaService {
  ...
  private async validarPeriodo(id: string, data: string, escrita: boolean) { const t: any = await this.repository.buscarTurma(id); if (!t) throw erroFrequencia.naoEncontrado("Turma/disciplina não encontrada."); const dia = new Date(`${data}T12:00:00-03:00`); if (dia > new Date()) throw erroFrequencia.invalido("Não é permitido registrar data futura."); if (data < this.iso(t.data_inicio) || data > this.iso(t.data_fim)) throw erroFrequencia.invalido("Data fora do período letivo da turma."); if (escrita && (!t.periodo_ativo || !["ativo", "ATIVO", "em_andamento", "EM_ANDAMENTO"].includes(t.periodo_status))) throw erroFrequencia.conflito("O período letivo não está ativo para lançamentos."); return t; }
  ...
  private data(v: string) { if (!DATA.test(v) || Number.isNaN(new Date(`${v}T12:00:00-03:00`).getTime())) throw erroFrequencia.invalido("Data inválida. Use AAAA-MM-DD."); }
