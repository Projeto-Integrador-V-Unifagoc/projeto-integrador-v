...
const ids = {
  ...
  alunoTurma1: "00000000-0000-0000-0000-000000000301",
  alunoTurma2: "00000000-0000-0000-0000-000000000302",
  alunoTurma3: "00000000-0000-0000-0000-000000000303",
};

export async function seed(knex: Knex): Promise<void> {
  await knex(`${SCHEMA}.cidade`).insert({ id: "00000000-0000-0000-0000-000000000401", nome: "Caratinga", uf: "MG", ibge: ids.cidadeIbge }).onConflict("ibge").ignore();

  await knex(`${SCHEMA}.usuario`).insert([
  ...
  await knex(`${SCHEMA}.faculdade`).insert({ id: ids.faculdade, nome: "Faculdade Demo Frequencia", cidade_id: ids.cidadeIbge, logradouro: "Avenida Universitaria", numero: "100", bairro: "Universitario", cep: "35300000" }).onConflict("id").ignore();
  await knex(`${SCHEMA}.departamento`).insert({ id: ids.departamento, codigo: "DEP-FREQ", nome: "Departamento Demo Frequencia", faculdade_id: ids.faculdade }).onConflict("id").ignore();
  await knex(`${SCHEMA}.curso`).insert({ id: ids.curso, codigo: "ADS-FREQ", nome: "Analise e Desenvolvimento de Sistemas", departamento_id: ids.departamento }).onConflict("id").ignore();
  await knex(`${SCHEMA}.disciplinas`).insert({ id: ids.disciplina, codigo: "PI5-FREQ", nome: "Projeto Integrador V", curso_id: ids.curso, carga_horaria: 80 }).onConflict("id").ignore();
  await knex(`${SCHEMA}.professor`).insert({ id: ids.professor, usuario_id: ids.usuarioProfessor, pessoa_id: ids.pessoaProfessor, curso_id: ids.curso, faculdade_id: ids.faculdade }).onConflict("id").ignore();
  await knex(`${SCHEMA}.local`).insert({ id: ids.local, codigo: "SALA-FREQ-DEMO" }).onConflict("id").ignore();
  await knex(`${SCHEMA}.turma`).insert({ id: ids.turma, disciplina_id: ids.disciplina, semestre: "2026/1", capacidade_alunos: 50, professor_id: ids.professor, curso_id: ids.curso }).onConflict("id").ignore();
  ...
  await knex(`${SCHEMA}.aluno_turma`).insert([
    { id: ids.alunoTurma1, aluno_id: ids.aluno1, turma_id: ids.turma, professor_id: ids.professor, status: "ATIVO", frequencia: 100 },
    { id: ids.alunoTurma2, aluno_id: ids.aluno2, turma_id: ids.turma, professor_id: ids.professor, status: "ATIVO", frequencia: 100 },
    { id: ids.alunoTurma3, aluno_id: ids.aluno3, turma_id: ids.turma, professor_id: ids.professor, status: "ATIVO", frequencia: 100 },
  ]).onConflict("id").ignore();
}
