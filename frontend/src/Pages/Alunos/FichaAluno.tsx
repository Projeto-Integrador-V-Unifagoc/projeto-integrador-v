...
function montarNotasFicha(
  ...
  const notasDaApi: NotaComSemestre[] = notasApiResponse.map((nota) => {
    ...
    return {
      ...
      avaliacao: nota.avaliacoes.reduce(
        (total: number, avaliacao: any) => total + Number(avaliacao.nota ?? 0),
        0,
      ),
  ...
  const notasPorMatricula: NotaComSemestre[] = matriculas
    .filter(
      (matricula) =>
        !disciplinasConhecidas.has((matricula as any).disciplina_nome),
    )
    .map((matricula) => ({
      disciplina: (matricula as any).disciplina_nome,
      mediaFinal: 0,
      avaliacao: 0,
      provaFinal: 0,
      provaInova: 0,
